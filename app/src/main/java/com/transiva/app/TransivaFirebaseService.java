...
public class TransivaFirebaseService extends FirebaseMessagingService {
  ...
  private void showHighPriorityNotification(
    ...
    Intent openIntent = buildOpenMainActivityIntent(
      screen,
      orderDbId,
      "",
      "",
    ...
    if (data != null && "1".equals(getValue(data, "has_action", ""))) {
      ...
      if (!orderDbId.isEmpty() && !token.isEmpty()) {
        ...
        Log.e(
          TAG,
          "Action tidak dibuat. orderDbId/action_token kosong. orderDbId=" +
            orderDbId
        );
      }
    }
  ...
  private PendingIntent createActionIntent(
    ...
    Intent intent = buildOpenMainActivityIntent(
      openScreen,
      orderDbId,
      action,
      endpoint,
  ...
  private Intent buildOpenMainActivityIntent(
    ...
    String safeDriverType = safe(driverType).isEmpty()
      ? "bike"
      : safe(driverType);

...
public class DriverNavigationActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle b){
        ...
        try{
            ...
            if(Build.VERSION.SDK_INT >= 23){
                // Keep system bars usable, but let the map itself occupy the complete app window.
    ...
    private String fullMapHtml(){
        ...
        return "<!doctype html><html><head><meta name='viewport' content='width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no'>"+
                ...
                "<style>html,body{height:100%;margin:0;background:#EAF4FF;overflow:hidden}#viewport{position:absolute;inset:0;overflow:hidden;background:#EAF4FF}#mapRotator{position:absolute;left:-65%;top:-65%;width:230%;height:230%;transform-origin:50% 50%;transition:transform .18s linear;will-change:transform}#map{height:100%;width:100%}.leaflet-control-attribution,.leaflet-control-zoom{display:none!important}.leaflet-container{font-family:Arial,sans-serif;background:#EAF4FF}.motorWrap{width:64px;height:64px;display:flex;align-items:center;justify-content:center}.motorWrap img{width:62px;height:62px;transform-origin:center center;filter:drop-shadow(0 6px 12px rgba(0,0,0,.35))}.topBadge{position:absolute;z-index:999;top:20px;left:76px;right:14px;background:rgba(255,255,255,.96);border:1px solid #D7E6F8;border-radius:18px;padding:12px 14px;color:#0B3A78;font-size:14px;font-weight:800;box-shadow:0 8px 24px rgba(15,23,42,.18)}.centerDot{position:absolute;z-index:998;left:50%;top:50%;width:8px;height:8px;margin-left:-4px;margin-top:-4px;border-radius:50%;background:#087CFF;box-shadow:0 0 0 10px rgba(8,124,255,.16);pointer-events:none}.speedBadge{position:absolute;z-index:1001;left:16px;bottom:24px;background:rgba(7,20,38,.88);color:white;border-radius:18px;padding:10px 14px;font:800 18px Arial;box-shadow:0 5px 18px rgba(0,0,0,.2)}.speedBadge small{font-size:11px;font-weight:600;opacity:.85;display:block;margin-top:2px}.fixedPin{width:58px;height:58px;transform-origin:29px 55px;will-change:transform}.fixedPin img{display:block;width:58px;height:58px}</style></head><body><div id='viewport'><div id='mapRotator'><div id='map'></div></div></div><div id='badge' class='topBadge'>Menyiapkan navigasi...</div><div id='speed' class='speedBadge'>0 km/j<small>Rata-rata 0 km/j</small></div><div class='centerDot'></div><script>"+
                ...
                "function fixedPinIcon(url){return L.divIcon({className:'',iconSize:[58,58],iconAnchor:[29,55],html:'<div class="fixedPin"><img src="'+url+'" style="width:58px;height:58px"></div>'});}var pickupIcon=fixedPinIcon('file:///android_res/drawable/map_pickup_pin.png');var destIcon=fixedPinIcon('file:///android_res/drawable/map_destination_pin.png');"+

...
public class CustomerTopUpActivity extends Activity {
    ...
    private void loadSession() {
        try {
            ...
            if (session.isLoggedIn()) {
                ...
                role = firstNonEmpty(session.getRole(), "customer");
                return;
            }
        } catch (Exception ignored) {}
        ...
        try {
            ...
            role = firstNonEmpty(sp.getString("role", ""), sp.getString("player_role", ""), "customer");
        } catch (Exception ignored) {}
    }
    ...
    private void buildTopBar() {
        ...
        col.addView(text("Isi Saldo", 23, "#0B3A78", true));
        col.addView(text("Top up Transiva Pay lewat QRIS", 12, "#64748B", false));
    }
    ...
    private void buildBalanceCard() {
        ...
        card.addView(text("💳 Saldo Saat Ini", 13, "#EAF4FF", true));
        balanceText = text("Memuat saldo...", 26, "#FFFFFF", true);
        LinearLayout.LayoutParams bLp = new LinearLayout.LayoutParams(-1, -2);
        bLp.setMargins(0, dp(6), 0, 0);
        card.addView(balanceText, bLp);

        TextView user = text(username, 12, "#EAF4FF", false);
        user.setPadding(0, dp(8), 0, 0);
        card.addView(user);
    }
    ...
    private void buildAmountCard() {
        ...
        card.addView(amountInput, new LinearLayout.LayoutParams(-1, dp(56)));
        ...
        addQuickAmount(quick, 10000);
        addQuickAmount(quick, 25000);
        addQuickAmount(quick, 50000);

        Button next = primaryButton("Lanjut Pembayaran");
    ...
    private void buildQrisCard(boolean visible) {
        ...
        TextView info = text("Transfer sesuai nominal, lalu upload bukti pembayaran.", 12, "#64748B", false);
        ...
        amountTitle.setPadding(0, dp(14), 0, 0);
        card.addView(amountTitle);

        TextView selectedAmountText = text("-", 24, "#0B7CFF", true);
        selectedAmountText.setTag("selected_amount_text");
        selectedAmountText.setPadding(0, dp(2), 0, dp(12));
        card.addView(selectedAmountText);

        Button pick = outlineButton("Upload Bukti Transfer");
        card.addView(pick, new LinearLayout.LayoutParams(-1, dp(52)));
        pick.setOnClickListener(v -> pickProofImage());

        proofNameText = text("Belum ada bukti dipilih", 12, "#64748B", false);
        proofNameText.setGravity(Gravity.CENTER);
        proofNameText.setPadding(0, dp(10), 0, dp(10));
        card.addView(proofNameText);

        submitButton = primaryButton("Kirim Bukti Deposit");
        card.addView(submitButton, new LinearLayout.LayoutParams(-1, dp(52)));
        submitButton.setOnClickListener(v -> submitDeposit());

        statusText = text("Menunggu upload bukti pembayaran", 13, "#64748B", false);
        statusText.setGravity(Gravity.CENTER);
        statusText.setPadding(dp(12), dp(14), dp(12), dp(14));
        statusText.setBackground(roundStroke("#F8FBFF", "#D7E6F8", dp(18), 1));
        LinearLayout.LayoutParams sLp = new LinearLayout.LayoutParams(-1, -2);
        sLp.setMargins(0, dp(12), 0, 0);
        card.addView(statusText, sLp);
    }

    private void showQris() {
        int amount = parseInt(firstNonEmpty(amountInput.getText().toString(), "0"));
        if (amount < MIN_DEPOSIT) {
            showInfo("Nominal Tidak Valid", "Minimal deposit adalah Rp 10.000");
            return;
        }
        ...
        View qrisCard = root.findViewWithTag("qris_card");
        if (qrisCard != null) qrisCard.setVisibility(View.VISIBLE);
        TextView amountView = root.findViewWithTag("selected_amount_text");
        if (amountView != null) amountView.setText(rupiah(amount));
        loadQrisImage();
    }
    ...
    private void loadQrisImage() {
        ...
        new Thread(() -> {
            ...
            mainHandler.post(() -> {
                setLoading(false);
                if (finalBm != null) qrisImage.setImageBitmap(finalBm);
                else showInfo("QRIS", "Gagal memuat gambar QRIS. Periksa koneksi internet.");
            });
        }).start();
    }
    ...
    private void pickProofImage() {
        try {
            ...
            startActivityForResult(Intent.createChooser(intent, "Pilih bukti transfer"), REQ_PICK_IMAGE);
        } catch (Exception e) {
            showInfo("Upload Bukti", "Tidak bisa membuka galeri/file manager.");
        }
    }
    ...
    private void submitDeposit() {
        if (selectedAmount < MIN_DEPOSIT) {
            showInfo("Nominal Tidak Valid", "Tekan Lanjut Pembayaran setelah mengisi nominal.");
            return;
        }
        if (proofUri == null) {
            showInfo("Bukti Belum Ada", "Upload bukti transfer terlebih dahulu.");
            return;
        }
        if (username == null || username.trim().length() == 0 || "User".equals(username)) {
            showInfo("Sesi Tidak Valid", "Data username tidak ditemukan. Silakan login ulang.");
            return;
        }
        ...
        new Thread(() -> {
            try {
                ...
                mainHandler.post(() -> {
                    setLoading(false);
                    setSubmitEnabled(true);
                    setStatus("❌ Koneksi bermasalah saat mengirim bukti", false);
                    showInfo("Koneksi Bermasalah", "Terjadi kesalahan saat mengirim bukti deposit. Periksa koneksi internet lalu coba lagi.");
                });
            }
        }).start();
    }
    ...
    private JSONObject uploadDeposit(Uri fileUri, int amount) throws Exception {
        ...
        try {
            ...
            writeField(out, boundary, "role", role);
            writeField(out, boundary, "amount", String.valueOf(amount));
            writeFile(out, boundary, "proof", getFileName(fileUri), fileUri);
    ...
    private void writeFile(OutputStream out, String boundary, String fieldName, String fileName, Uri uri) throws Exception {
        String mime = firstNonEmpty(getContentResolver().getType(uri), "image/jpeg");
        ...
        while ((len = in.read(buffer)) != -1) {
            ...
            if (total > 5L * 1024L * 1024L) {
                in.close();
                throw new Exception("Ukuran maksimal 5MB");
            }
    ...
    private void loadBalance() {
        ...
        new Thread(() -> {
            ...
            mainHandler.post(() -> {
                setLoading(false);
                if (balanceText != null) balanceText.setText(finalResult);
            });
        }).start();
    }
    ...
    private String sanitizeFileName(String name) {
        return firstNonEmpty(name, "bukti_deposit.jpg").replaceAll("[^a-zA-Z0-9._-]", "_");
    }

    private void setStatus(String message, boolean success) {
        if (statusText == null) return;
        statusText.setText(message);
        statusText.setTextColor(Color.parseColor(success ? "#0B7C55" : "#64748B"));
        statusText.setBackground(roundStroke(success ? "#ECFDF5" : "#F8FBFF", success ? "#A7F3D0" : "#D7E6F8", dp(18), 1));
    }

    private void setSubmitEnabled(boolean enabled) {
        if (submitButton != null) {
            submitButton.setEnabled(enabled);
            submitButton.setText(enabled ? "Kirim Bukti Deposit" : "Mengirim...");
            submitButton.setAlpha(enabled ? 1f : 0.65f);
        }
    }
    ...
    private int parseInt(String value) {
        try { return Integer.parseInt(firstNonEmpty(value, "0").replace(".", "").replace(",", "")); }
